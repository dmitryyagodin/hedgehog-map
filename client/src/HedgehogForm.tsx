...
export function HedgehogForm() {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const validation = hedgehogSchema.safeParse({ name, age, gender, coordinates });
    ...
    return (
      <form onSubmit={handleSubmit} >
        <Box display="flex" flexDirection="column" rowGap={2}>
          <TextField label="Nimi" name="name" variant="outlined" error={!!errors.name} helperText={errors.name} fullWidth />
          <TextField label="Ikä (1-20)" name="age" variant="outlined" type="number" error={!!errors.age} helperText={errors.age} fullWidth placeholder='1-20' />
          <FormControl variant="outlined" error={!!errors.gender} fullWidth>
            <InputLabel id="gender-label">Sukupuoli</InputLabel>
            <Select labelId="gender-label" label="Sukupuoli" name="gender" variant="outlined" error={!!errors.gender} defaultValue="">
              <MenuItem value="unknown" selected>Tunematon</MenuItem>
              <MenuItem value="male">Koiras</MenuItem>
              <MenuItem value="female">Naaras</MenuItem>
            </Select>
          </FormControl>
          <TextField label="Lat." variant="outlined" value={latitude} disabled fullWidth />
          <TextField label="Long." variant="outlined" value={longitude} disabled fullWidth />
        </Box>
        <Button type="submit" variant="contained" color="primary" fullWidth>Lisää</Button>
      </form>
    );
  }
